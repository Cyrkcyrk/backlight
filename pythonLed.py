...
print("ZONE_W: ", ZONE_W, "ZONE_H", ZONE_H)
print("SPLIT_W: ", SPLIT_W, "SPLIT_H", SPLIT_H)
# screen_sample = [[[255, 0, 0] for x in range(SPLIT_W)] for y in range(SPLIT_H)]
screen_sample = np.zeros((SPLIT_H, SPLIT_W, 3), dtype=np.uint8);
# image = Image.fromarray(screen_sample)
# image.show()
...
for i in range(0, SPLIT_W):
    for j in range(0, SPLIT_H):
        if (j == 0 or j == SPLIT_H-1 or i == 0 or i == SPLIT_W-1):
            ...
            screen_sample[j, i][2] = round(((mean_col[0] * ZONE_STEP * ZONE_STEP) / (ZONE_H * ZONE_W)))
            screen_sample[j, i][1] = round(((mean_col[1] * ZONE_STEP * ZONE_STEP) / (ZONE_H * ZONE_W)))
            screen_sample[j, i][0] = round(((mean_col[2] * ZONE_STEP * ZONE_STEP) / (ZONE_H * ZONE_W)))
image = Image.fromarray(screen_sample)
image.show()
